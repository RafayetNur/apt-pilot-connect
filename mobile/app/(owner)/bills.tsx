...
import { useOwnerBuildings, currentMonthInput, formatBDT } from "@/lib/owner/shared";
import {
  bulkChargeTypes,
  flatChargeLabel,
  isMonthClosed,
...
/**
 * Owner bill entry. Flat-bill and shared-charge entry rules are identical
 * between owner and manager (same `flat_bill_charges` / `shared_building_
 * charges` tables, same `allocate_shared_charge` RPC, same "locked once a
 * payment is pending/verified" rule) — this screen reuses
 * mobile/lib/manager/bills.ts directly rather than duplicating it, and is
 * visually the same as mobile/app/(manager)/bills.tsx (which mirrors the
 * web app's src/components/bills/bills-page.tsx). Month close/reopen itself
 * is handled on the Rent screen since only the owner may call it.
 */
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {buildingsLoading ? (
        ...
        <Text style={[styles.emptyText, { color: colors.textSub, padding: 20 }]}>No buildings yet.</Text>
      ) : (
        <>
          ...
          {monthClosed ? (
            <View style={[styles.lockedBanner, { backgroundColor: colors.warningBg, borderColor: colors.warning }]}>
              ...
              <Text style={[styles.lockedText, { color: colors.warning }]} maxFontSizeMultiplier={1.3}>
                This billing month is closed. Reopen it from the Rent screen before changing bills.
              </Text>
            </View>
          ) : null}
          ...
          >
            ...
            <View style={styles.sharedHeaderRow}>
              ...
              {!monthClosed ? (
                <TouchableOpacity style={[styles.addChip, { backgroundColor: colors.primary }]} onPress={() => setSharedModalVisible(true)}>
              ...
              <View style={[styles.list, { marginBottom: 40 }]}>
                {charges.map((charge) => (
                  <View key={charge.id} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
                    ...
                    {charge.description ? <Text style={[styles.chargesSummary, { color: colors.textSub }]}>{charge.description}</Text> : null}
                    <Text style={[styles.chargesSummary, { color: colors.textSub }]}>
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
          <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
            ...
            <TouchableOpacity style={[styles.submitBtn, { backgroundColor: colors.primary }, saving && styles.disabled]} onPress={handleSave} disabled={saving}>
              {saving ? (
