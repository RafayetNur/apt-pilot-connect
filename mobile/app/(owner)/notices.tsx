...
import { formatDateSafe, useOwnerBuildings } from "@/lib/owner/shared";
import {
...
/**
 * Owner notice board. The notice workflow is identical between owner and
 * manager (same `notice_create` + `notice_publish` RPCs, same audience
 * options) — this screen reuses mobile/lib/manager/notices.ts directly, and
 * is visually the same as mobile/app/(manager)/notices.tsx (itself ported
 * from the Sanjida reference's app/(owner)/notices.tsx / app/(manager)/
 * notices.tsx header+FAB layout). Only "all tenants" and "owner/manager
 * only" audiences are offered — see the manager notices module's own
 * comment for why selected-flats/selected-tenants targeting is left out of
 * this pass.
 */
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
            ...
            <View style={styles.chipRow}>
              {noticePriorityOptions.map((option) => (
                <TouchableOpacity
                  key={option}
                  style={[styles.chip, { backgroundColor: colors.background, borderColor: colors.border }, priority === option && { backgroundColor: colors.primary + "20", borderColor: colors.primary }]}
            ...
            <View style={styles.chipRow}>
              {(["all_tenants", "owner_manager_only"] as const).map((option) => (
                <TouchableOpacity
                  key={option}
                  style={[styles.chip, { backgroundColor: colors.background, borderColor: colors.border }, audience === option && { backgroundColor: colors.primary + "20", borderColor: colors.primary }]}
