...
import { useOwnerBuildings, formatBDT, formatDateTimeSafe, formatMonthLabel } from "@/lib/owner/shared";
import {
...
/**
 * Owner review of tenant rent-payment submissions. The review workflow is
 * identical between owner and manager (same `rent_payments` query, same
 * `review_rent_payment` RPC, same payment-proofs bucket) — this screen
 * reuses mobile/lib/manager/payments.ts directly rather than duplicating it,
 * and is visually the same as mobile/app/(manager)/payments.tsx.
 */
  ...
  return (
    <TouchableOpacity style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]} onPress={onPress}>
      <View style={styles.cardHeader}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.flatText, { color: colors.text }]}>{payment.building_name} · Flat {payment.flat_number}</Text>
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
          <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
            ...
            {payment.reviewer_note ? <Text style={[styles.detailLine, { color: colors.textSub }]}>Previous note: {payment.reviewer_note}</Text> : null}

            {payment.payment_proof_url ? (
              proofUrl ? (
                <Image source={{ uri: proofUrl }} style={styles.proofImage} resizeMode="contain" />
              ) : (
                <TouchableOpacity style={[styles.proofButton, { backgroundColor: colors.surface, borderColor: colors.border }]} onPress={loadProof} disabled={proofLoading}>
            ...
            {canAct ? (
              <>
                ...
                <View style={styles.actionsRow}>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.success }]} onPress={() => handleAction("verify")} disabled={submittingAction !== null}>
                    {submittingAction === "verify" ? <ActivityIndicator color="#fff" /> : <CheckCircle2 color="#fff" size={16} />}
                    <Text style={styles.actionBtnText}>Verify</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.warning }]} onPress={() => handleAction("correction_requested")} disabled={submittingAction !== null}>
                  ...
                  <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.danger }]} onPress={() => handleAction("reject")} disabled={submittingAction !== null}>
