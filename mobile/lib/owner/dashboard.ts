...
/**
 * Owner dashboard summary. Mirrors the web app's
 * src/lib/dashboard.ts::dashboardSummaryQueryOptions query-for-query (same
 * as the manager mobile dashboard in lib/manager/dashboard.ts, plus
 * tenant_credits and an open-maintenance count, matching the web owner
 * dashboard's extra "Tenant advance credit" stat and the
 * MaintenanceDashboardSection it renders): buildings / flats / rent_records /
 * rent_payments / tenant_credits / maintenance_requests are fetched with no
 * manual "owner" filter because RLS already scopes every one of those tables
 * to buildings owned by this user. Only the totals math is duplicated
 * client-side (grouping and summing plain numbers already computed by the
 * rent_records triggers) — no accounting logic is recomputed.
 */
...
const openMaintenanceStatuses = ["submitted", "acknowledged", "assigned", "in_progress", "waiting_for_parts", "reopened"];

function num(value: unknown) {
  return Number(value ?? 0);
}

export function useOwnerDashboard(monthInput: string) {
  const { session } = useAuth();
  const state = useAsyncState<OwnerDashboardData | null>(null);
  ...
  const load = useCallback(
    async (isRefresh = false) => {
      ...
      const [buildingsRes, flatsRes, recordsRes, paymentsRes, creditsRes, maintenanceRes] = await Promise.all([
      ...
      const loadError =
        buildingsRes.error ?? flatsRes.error ?? recordsRes.error ?? paymentsRes.error ?? creditsRes.error ?? maintenanceRes.error;
      if (loadError) {
      ...
      const maintenanceRows = (maintenanceRes.data ?? []) as { id: string; status: string; priority: string }[];
      const openMaintenanceRows = maintenanceRows.filter((row) => openMaintenanceStatuses.includes(row.status));
      ...
      state.setData({
        totals: {
          ...
          emergencyMaintenance: openMaintenanceRows.filter((row) => row.priority === "emergency").length,
        },
        buildingSummaries,
        needsChargeEntry: records.filter((row) => row.individual_charges_total === 0),
        outstanding: records.filter((row) => row.remaining_due > 0).sort((a, b) => b.remaining_due - a.remaining_due),
